...
<div x-data="contactSearch_{{ $componentId }}()" class="relative">
    ...
    <input type="text" x-model="query" @input.debounce.300ms="search()" @focus="if(query.length >= 1 || results.length) open = true; else { search(); }"
        placeholder="Kontakt suchen..." class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500 mb-2">
    ...
    <div x-show="open && results.length === 0 && query.length >= 1 && !loading" x-cloak class="absolute z-10 w-full bg-white border border-gray-200 rounded-lg shadow-lg p-3">
        <p class="text-sm text-gray-500">Keine Kontakte gefunden.</p>
    </div>
    ...
    <div x-show="showCreateForm" x-cloak class="mt-2 p-3 bg-gray-50 border border-gray-200 rounded-lg space-y-2">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <input type="text" x-model="newFirstName" placeholder="Vorname *" class="rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
            <input type="text" x-model="newLastName" placeholder="Nachname *" class="rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
        </div>

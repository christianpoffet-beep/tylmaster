...
@php
    ...
    $orgTypeColorsSearch = ['band' => 'bg-purple-100 text-purple-700', 'label' => 'bg-blue-100 text-blue-700', 'publishing' => 'bg-indigo-100 text-indigo-700', 'venue' => 'bg-green-100 text-green-700', 'event_festival' => 'bg-yellow-100 text-yellow-700', 'media' => 'bg-pink-100 text-pink-700', 'oma' => 'bg-gray-100 text-gray-600'];
@endphp

<div x-data="organizationSearch()" class="relative">
    <label class="block text-sm font-medium text-gray-700 mb-1">Organisationen</label>
    ...
    <div x-show="open && results.length > 0" @click.away="open = false" x-cloak
        ...
        <template x-for="result in results" :key="result.id">
            <button type="button" @click="addOrganization(result)"
                ...
                <div>
                    <span class="text-sm text-gray-900" x-text="result.primary_name"></span>
                    <span x-show="result.all_names !== result.primary_name" class="text-xs text-gray-400 ml-1" x-text="'(' + result.all_names + ')'"></span>
                </div>
                <span class="text-xs px-1.5 py-0.5 rounded" :class="typeColorMap[result.type] || 'bg-gray-100 text-gray-600'" x-text="typeLabelMap[result.type] || result.type"></span>
            </button>
        </template>
    </div>
    ...
    <div x-show="selected.length > 0" class="flex flex-wrap gap-2 mt-2">
        <template x-for="org in selected" :key="org.id">
            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium" :class="typeColorMap[org.type] || 'bg-gray-100 text-gray-600'">
    ...
    {{-- Quick create toggle --}}
    <button type="button" @click="showCreateForm = !showCreateForm" class="mt-2 inline-flex items-center text-sm text-blue-600 hover:text-blue-800">
    ...
    <div x-show="showCreateForm" x-cloak class="mt-2 p-3 bg-gray-50 border border-gray-200 rounded-lg space-y-2">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <select x-model="newType" class="rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                <option value="">Typ wählen *</option>
                @foreach($orgTypeLabelsSearch as $v => $l)
                    <option value="{{ $v }}">{{ $l }}</option>
                @endforeach
            </select>
            <input type="text" x-model="newName" placeholder="Name *" class="rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
        </div>

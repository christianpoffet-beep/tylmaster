...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
    ...
    <a href="{{ route('admin.contracts.create') }}" class="px-4 py-2 bg-blue-600 dark:bg-blue-700 text-white text-sm rounded-lg hover:bg-blue-700 dark:hover:bg-blue-600 whitespace-nowrap">+ Neuer Vertrag</a>
</div>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700/50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Parteien</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($contracts as $contract)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 dark:bg-gray-700/50">
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $typeColors[$contract->type] ?? 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300' }}">{{ $typeLabels[$contract->type] ?? ucfirst($contract->type) }}</span>
                        </td>
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $statusColors[$contract->status] ?? 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300' }}">{{ $statusLabels[$contract->status] ?? $contract->status }}</span>
                        </td>
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('admin.contracts.edit', $contract) }}" class="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">Bearbeiten</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">Keine Verträge gefunden.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>

    ...
    <form method="POST" action="{{ route('admin.artworks.update', $artwork) }}" enctype="multipart/form-data">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $artwork->title) }}" required class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <hr class="border-gray-200">
            <h3 class="text-sm font-semibold text-gray-700">Artwork-Bild</h3>
            ...
            <div>
                <label for="artwork_file" class="block text-sm font-medium text-gray-700 mb-1">{{ $artwork->artwork_path ? 'Artwork ersetzen' : 'Artwork hochladen' }}</label>
            ...
            <hr class="border-gray-200">
            <h3 class="text-sm font-semibold text-gray-700">Credits</h3>
            ...
            <div>
                <label for="yoc" class="block text-sm font-medium text-gray-700 mb-1">YOC (Year of Creation)</label>
                <input type="number" name="yoc" id="yoc" value="{{ old('yoc', $artwork->yoc) }}" min="1900" max="{{ date('Y') + 1 }}" class="w-32 rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                @error('yoc') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <hr class="border-gray-200">
            <h3 class="text-sm font-semibold text-gray-700">Projekte</h3>
            ...
            <hr class="border-gray-200">
            <h3 class="text-sm font-semibold text-gray-700">Bestehende Logos</h3>
                ...
                <div class="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg">
                    <div class="flex items-center gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            @if($logo->comment)
                                <p class="text-xs text-gray-400 truncate">{{ $logo->comment }}</p>
                            @endif
                        </div>
                    </div>
            ...
            <h3 class="text-sm font-semibold text-gray-700">Neue Logos hinzufügen</h3>
            ...
            <div x-data="{ logos: [{ comment: '' }] }">
                ...
                <template x-for="(logo, index) in logos" :key="index">
                    <div class="flex flex-col sm:flex-row gap-2 mb-3 p-3 bg-gray-50 rounded-lg">
                        ...
                        <div class="flex-1">
                            <input type="text" :name="'logos[' + index + '][comment]'" x-model="logo.comment" placeholder="Kommentar (optional)" class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                ...
                <button type="button" @click="logos.push({ comment: '' })" class="inline-flex items-center text-sm text-blue-600 hover:text-blue-800">
        ...
        <div class="mt-4 flex gap-3">
            ...
            <a href="{{ route('admin.artworks.show', $artwork) }}" class="px-5 py-2.5 bg-white text-gray-700 text-sm font-medium rounded-lg border border-gray-300 hover:bg-gray-50">Abbrechen</a>
        </div>
    </form>

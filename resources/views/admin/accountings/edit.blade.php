    ...
    <form method="POST" action="{{ route('admin.accountings.update', $accounting) }}">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $accounting->name) }}" required class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="period_start" id="period_start" value="{{ old('period_start', $accounting->period_start->format('Y-m-d')) }}" required class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
                <div>
                    <label for="period_end" class="block text-sm font-medium text-gray-700 mb-1">Periode bis *</label>
                    <input type="date" name="period_end" id="period_end" value="{{ old('period_end', $accounting->period_end->format('Y-m-d')) }}" required class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
            </div>

            <div>
                <label for="notes" class="block text-sm font-medium text-gray-700 mb-1">Notizen</label>
                <textarea name="notes" id="notes" rows="3" class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">{{ old('notes', $accounting->notes) }}</textarea>
            </div>
        </div>
        ...
        <div class="mt-4 flex items-center gap-3">
            ...
            <a href="{{ route('admin.accountings.show', $accounting) }}" class="px-5 py-2.5 bg-white text-gray-700 text-sm font-medium rounded-lg border border-gray-300 hover:bg-gray-50">Abbrechen</a>
        </div>
    </form>

        ...
        <div x-show="step === 1">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
                ...
                <div>
                    ...
                    <select name="chart_template_id" id="chart_template_id" x-model="templateId" class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                <div x-show="templateId" x-cloak x-transition>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Eröffnungssaldi &amp; Kontenplan</label>
                    <div class="space-y-2">
                        <label class="flex items-start gap-2 p-3 rounded-lg border cursor-pointer transition-colors" :class="openingMode === 'zero' ? 'border-blue-300 bg-blue-50' : 'border-gray-200 hover:bg-gray-50'">
                            <input type="radio" value="zero" x-model="openingMode" class="mt-0.5 text-blue-600 focus:ring-blue-500">
                            <div>
                                <span class="text-sm font-medium text-gray-700">Vorlage direkt übernehmen</span>
                                <p class="text-xs text-gray-500 mt-0.5">Alle Konten starten bei 0. Du kannst sie danach anpassen.</p>
                            </div>
                        </label>
                        <label class="flex items-start gap-2 p-3 rounded-lg border cursor-pointer transition-colors" :class="openingMode === 'manual' ? 'border-blue-300 bg-blue-50' : 'border-gray-200 hover:bg-gray-50'">
                            <input type="radio" value="manual" x-model="openingMode" class="mt-0.5 text-blue-600 focus:ring-blue-500">
                            <div>
                                <span class="text-sm font-medium text-gray-700">Kontenplan anpassen</span>
        ...
        <div x-show="step === 2" x-cloak>
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                    <h3 class="text-sm font-semibold text-gray-900">Kontenplan anpassen</h3>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase w-20">Nr.</th>
                                <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">Bezeichnung</th>
                                <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase w-28">Typ</th>
                                <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase w-36">Eröffnungssaldo</th>
                                    ...
                                    <td class="px-3 py-2 text-right">
                                        <template x-if="!account.is_header && (account.type === 'asset' || account.type === 'liability')">
                                            <input type="number" step="0.01"
                                                ...
                                                class="w-full text-right rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500 font-mono">
                                        </template>
                                        <template x-if="!account.is_header && account.type !== 'asset' && account.type !== 'liability'">
                                            <span class="text-sm text-gray-400 font-mono">0.00</span>
                                        </template>
                                    </td>
                ...
                <div class="px-4 py-4 bg-gray-50 border-t border-gray-200">
                    <p class="text-xs font-medium text-gray-500 uppercase mb-2">Konto hinzufügen</p>
                    <div class="flex flex-wrap items-end gap-2">
                        <div>
                            <label class="block text-xs text-gray-500 mb-0.5">Nr.</label>
                            <input type="text" x-model="newAccount.number" placeholder="1000" class="w-20 rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                        <div class="flex-1 min-w-[120px]">
                            <label class="block text-xs text-gray-500 mb-0.5">Bezeichnung</label>
                            <input type="text" x-model="newAccount.name" placeholder="Kasse" class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                        <div>
                            <label class="block text-xs text-gray-500 mb-0.5">Typ</label>
                            <select x-model="newAccount.type" class="rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" x-model.number="newAccount.opening_balance" placeholder="0.00" class="w-28 rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500 font-mono text-right">
                        </div>
                        <button type="button" @click="addAccount()" class="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">+</button>
                    </div>
            ...
            <div class="mt-3 flex items-center gap-3">
                <button type="button" @click="step = 1" class="px-5 py-2.5 bg-white text-gray-700 text-sm font-medium rounded-lg border border-gray-300 hover:bg-gray-50">&larr; Zurück</button>
                <button type="submit" class="px-5 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700">Buchhaltung erstellen</button>
            </div>
        </div>
    </form>

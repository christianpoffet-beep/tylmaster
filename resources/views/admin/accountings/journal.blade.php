...
<form method="GET" action="{{ route('admin.accountings.journal', $accounting) }}" class="flex flex-wrap gap-2 mb-4 items-end">
    <input type="date" name="from" value="{{ request('from') }}" class="rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Von">
    <input type="date" name="to" value="{{ request('to') }}" class="rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Bis">
    @if($projects->count())
        <select name="project_id" class="rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
    ...
    @if($organizations->count())
        <select name="organization_id" class="rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
    ...
    @endif
    @if($contacts->count())
        <select name="contact_id" class="rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700/50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase w-24">Beleg-Nr.</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Beschreibung</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Soll</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Haben</th>
                    <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase w-32">Betrag</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($bookings as $booking)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 dark:bg-gray-700/50">
                        ...
                        <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-300">{{ $booking->debitAccount->number }} {{ $booking->debitAccount->name }}</td>
                        <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-300">{{ $booking->creditAccount->number }} {{ $booking->creditAccount->name }}</td>
                        <td class="px-4 py-2 text-sm text-right font-mono text-gray-900 dark:text-gray-100">{{ number_format($booking->amount, 2, '.', "'") }}</td>
                        <td class="px-4 py-2 text-right">
                            @if(!$accounting->is_closed)
                                <a href="{{ route('admin.bookings.edit', $booking) }}" class="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">Bearb.</a>
                            @endif
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">Keine Buchungen vorhanden.</td>
                    </tr>
                @endforelse
            </tbody>
            @if($bookings->count())
            <tfoot class="bg-gray-50 dark:bg-gray-700/50">
                <tr>
                    <td colspan="5" class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 text-right">Total:</td>
                    <td class="px-4 py-2 text-sm text-right font-mono font-bold text-gray-900 dark:text-gray-100">{{ number_format($bookings->sum('amount'), 2, '.', "'") }}</td>
                    <td></td>
                </tr>
            </tfoot>
            @endif

...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
    ...
    <div class="flex gap-2">
        <a href="{{ route('admin.accountings.balanceSheet', $accounting) }}" class="px-4 py-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 text-sm rounded-lg border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700">Bilanz</a>
...
<div class="mb-6 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div>
            ...
            <p class="text-2xl font-bold mt-1 {{ $profitLoss >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">{{ number_format(abs($profitLoss), 2, '.', "'") }} {{ $accounting->currency }}</p>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="px-4 py-3 bg-green-50 dark:bg-green-900/30 border-b border-green-100 dark:border-green-800">
            <h3 class="text-sm font-semibold text-green-800 dark:text-green-300">Ertrag</h3>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    @empty
                        <tr><td colspan="3" class="px-4 py-4 text-sm text-gray-400 dark:text-gray-500 text-center">Keine Ertragskonten vorhanden</td></tr>
                    @endforelse
                </tbody>
                <tfoot class="bg-gray-50 dark:bg-gray-700/50">
                    <tr class="font-bold">
                        <td colspan="2" class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">Total Ertrag</td>
                        <td class="px-4 py-3 text-sm text-right font-mono text-green-700 dark:text-green-400">{{ number_format($totalIncome, 2, '.', "'") }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
    ...
    {{-- Aufwand --}}
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="px-4 py-3 bg-red-50 dark:bg-red-900/30 border-b border-red-100 dark:border-red-800">
            <h3 class="text-sm font-semibold text-red-800 dark:text-red-300">Aufwand</h3>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700/50">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    @empty
                        <tr><td colspan="3" class="px-4 py-4 text-sm text-gray-400 dark:text-gray-500 text-center">Keine Aufwandskonten vorhanden</td></tr>
                    @endforelse
                </tbody>
                <tfoot class="bg-gray-50 dark:bg-gray-700/50">
                    <tr class="font-bold">
                        <td colspan="2" class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">Total Aufwand</td>
                        <td class="px-4 py-3 text-sm text-right font-mono text-red-700 dark:text-red-400">{{ number_format($totalExpenses, 2, '.', "'") }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
    ...
    <table class="min-w-full">
        <tbody>
            <tr class="border-b border-gray-200 dark:border-gray-700">
                ...
                <td class="px-6 py-3 text-sm text-right font-mono text-green-700 dark:text-green-400 font-medium">{{ number_format($totalIncome, 2, '.', "'") }}</td>
            </tr>
            <tr class="border-b border-gray-200 dark:border-gray-700">
                <td class="px-6 py-3 text-sm text-gray-600 dark:text-gray-300">Total Aufwand</td>
                <td class="px-6 py-3 text-sm text-right font-mono text-red-700 dark:text-red-400 font-medium">- {{ number_format($totalExpenses, 2, '.', "'") }}</td>
            </tr>
            <tr class="bg-gray-50 dark:bg-gray-700/50">
                <td class="px-6 py-4 text-sm font-bold {{ $profitLoss >= 0 ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400' }}">
                    {{ $profitLoss >= 0 ? 'Gewinn' : 'Verlust' }}
                </td>
                <td class="px-6 py-4 text-right font-mono text-lg font-bold {{ $profitLoss >= 0 ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400' }}">

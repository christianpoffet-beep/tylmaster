...
<div class="max-w-2xl">
    <div class="mb-6">
        <h2 class="text-xl font-bold text-gray-900">Buchung bearbeiten</h2>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="booking_date" id="booking_date" value="{{ old('booking_date', $booking->booking_date->format('Y-m-d')) }}" required class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <input type="text" name="reference" id="reference" value="{{ old('reference', $booking->reference) }}" class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
            </div>

            <div>
                <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Beschreibung *</label>
                <input type="text" name="description" id="description" value="{{ old('description', $booking->description) }}" required class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="debit_account_id" id="debit_account_id" required class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        @foreach($accounts as $acc)
                            <option value="{{ $acc->id }}" {{ old('debit_account_id', $booking->debit_account_id) == $acc->id ? 'selected' : '' }}>{{ $acc->number }} {{ $acc->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="credit_account_id" id="credit_account_id" required class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                        <option value="">— Konto wählen —</option>
                        @foreach($accounts as $acc)
                            <option value="{{ $acc->id }}" {{ old('credit_account_id', $booking->credit_account_id) == $acc->id ? 'selected' : '' }}>{{ $acc->number }} {{ $acc->name }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <input type="number" name="amount" id="amount" value="{{ old('amount', $booking->amount) }}" step="0.01" min="0.01" required class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                @error('amount') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="notes" class="block text-sm font-medium text-gray-700 mb-1">Notizen</label>
                <textarea name="notes" id="notes" rows="2" class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">{{ old('notes', $booking->notes) }}</textarea>
            </div>
            ...
            {{-- Belege hochladen --}}
            <div class="border-t border-gray-200 pt-6">
                <label for="documents" class="block text-sm font-medium text-gray-700 mb-1">Neue Belege hochladen</label>
                ...
                @foreach($booking->documents as $doc)
                    <div class="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg">
                        <div class="flex items-center gap-3 min-w-0">
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-200 text-gray-700">{{ $doc->file_extension }}</span>
                        ...
                        <div class="flex items-center gap-2 flex-shrink-0">
                            <a href="{{ route('admin.documents.download', $doc) }}" class="text-xs text-blue-600 hover:text-blue-800">Download</a>

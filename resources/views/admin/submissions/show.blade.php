...
<div class="max-w-4xl">
            ...
            <div>
                ...
                <div class="mt-2 flex items-center gap-2">
                    ...
                    @elseif($submission->payment_status === 'pending')
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-yellow-100 dark:bg-yellow-900/50 text-yellow-700 dark:text-yellow-300">Zahlung ausstehend</span>
                    @endif
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <x-admin.collapsible-card title="Kontaktdaten">
            <dl class="space-y-3">
                <div class="flex justify-between"><dt class="text-sm text-gray-500 dark:text-gray-400">Name</dt><dd class="text-sm text-gray-900 dark:text-gray-100">{{ $submission->first_name }} {{ $submission->last_name }}</dd></div>
        ...
        <x-admin.collapsible-card title="Release-Details">
            <dl class="space-y-3">
                <div class="flex justify-between"><dt class="text-sm text-gray-500 dark:text-gray-400">Projektname</dt><dd class="text-sm text-gray-900 dark:text-gray-100">{{ $submission->project_name ?? '-' }}</dd></div>
        ...
        <x-admin.collapsible-card title="Media & Bio">
            <dl class="space-y-3">
                <div class="flex justify-between"><dt class="text-sm text-gray-500 dark:text-gray-400">Website</dt><dd class="text-sm text-gray-900 dark:text-gray-100">@if($submission->website)<a href="{{ $submission->website }}" target="_blank" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">{{ $submission->website }}</a>@else - @endif</dd></div>
                <div class="flex justify-between"><dt class="text-sm text-gray-500 dark:text-gray-400">Spotify</dt><dd class="text-sm text-gray-900 dark:text-gray-100">@if($submission->spotify_link)<a href="{{ $submission->spotify_link }}" target="_blank" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">Spotify-Profil</a>@else - @endif</dd></div>
                <div class="flex justify-between"><dt class="text-sm text-gray-500 dark:text-gray-400">Instagram</dt><dd class="text-sm text-gray-900 dark:text-gray-100">{{ $submission->instagram ?? '-' }}</dd></div>
                <div class="flex justify-between"><dt class="text-sm text-gray-500 dark:text-gray-400">Weitere Socials</dt><dd class="text-sm text-gray-900 dark:text-gray-100">{{ $submission->social_other ?? '-' }}</dd></div>
            </dl>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700/50">
                        <tr>
                            ...
                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Titel</th>
                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">ISRC</th>
                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Featuring</th>
                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Songwriter</th>
                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Producer</th>
                        </tr>
                    </thead>
    ...
    <x-admin.collapsible-card title="Vertragsdetails" class="mt-6">
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
            <div class="flex justify-between"><dt class="text-sm text-gray-500 dark:text-gray-400">Ausgeschlossene Länder</dt><dd class="text-sm text-gray-900 dark:text-gray-100">{{ $submission->contract_excluded_countries ?? '-' }}</dd></div>
            <div class="flex justify-between"><dt class="text-sm text-gray-500 dark:text-gray-400">Vertragsende</dt><dd class="text-sm text-gray-900 dark:text-gray-100">{{ $submission->contract_end_date?->format('d.m.Y') ?? '-' }}</dd></div>
            <div class="flex justify-between"><dt class="text-sm text-gray-500 dark:text-gray-400">Vorschuss-Interesse</dt><dd class="text-sm text-gray-900 dark:text-gray-100">{{ $submission->contract_advance_interest ?? '-' }}</dd></div>
            <div class="flex justify-between"><dt class="text-sm text-gray-500 dark:text-gray-400">Digitale Signatur</dt><dd class="text-sm text-gray-900 dark:text-gray-100">{{ $submission->digital_signature ?? '-' }}</dd></div>
            <div class="flex justify-between"><dt class="text-sm text-gray-500 dark:text-gray-400">Unterzeichnet am</dt><dd class="text-sm text-gray-900 dark:text-gray-100">{{ $submission->contract_sign_date?->format('d.m.Y') ?? '-' }}</dd></div>
        </dl>
    </x-admin.collapsible-card>
        ...
        <x-admin.collapsible-card title="Dateien" class="mt-6">
            <div class="flex flex-wrap gap-4">
                @if($submission->file_path)
                    <a href="{{ Storage::url($submission->file_path) }}" download class="inline-flex items-center px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm rounded-lg hover:bg-gray-200">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                        Audio-Datei
                    </a>
                @endif
                @if($submission->cover_image_path)
                    <a href="{{ Storage::url($submission->cover_image_path) }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm rounded-lg hover:bg-gray-200">

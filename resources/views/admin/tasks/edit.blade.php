    ...
    <form method="POST" action="{{ route('admin.tasks.update', $task) }}" enctype="multipart/form-data">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-6">
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $task->title) }}" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $task->description) }}</textarea>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    <label for="due_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Fällig am</label>
                    <input type="date" name="due_date" id="due_date" value="{{ old('due_date', $task->due_date?->format('Y-m-d')) }}" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
                <div>
                ...
                <div>
                    <label for="project_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Hauptprojekt</label>
            ...
            <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300">Vorhandene Dokumente</h3>
            <div class="space-y-2">
                @foreach($task->uploadedDocuments as $doc)
                    ...
                    <div class="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg">
                        <div class="flex items-center gap-2 flex-1 min-w-0">
                            ...
                            <div class="min-w-0">
                                @if($docPreviewable)
                                    <button type="button" class="text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 truncate block text-left"
                                ...
                                <div class="flex items-center gap-3 text-xs text-gray-500 mt-0.5">
                                    ...
                                    @if($doc->notes)
                                        <span class="text-gray-600 dark:text-gray-300">{{ $doc->notes }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
            ...
            {{-- New Document Upload --}}
            <hr class="border-gray-200 dark:border-gray-700">
            <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300">Neue Dokumente hochladen</h3>

            <div x-data="{ counter: 1 }">
                <div id="doc-rows-edit">
                    <div class="flex gap-2 items-start mb-3">
                        <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-2">
                            <input type="file" name="doc_files[]" class="w-full text-sm text-gray-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-gray-100 dark:file:bg-gray-600 file:text-gray-700 dark:file:text-gray-200 hover:file:bg-gray-200 dark:hover:file:bg-gray-500">
                            <input type="text" name="doc_notes[]" placeholder="Notiz zum Dokument..." class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                    </div>
                </div>
                <button type="button" @click="counter++; $nextTick(() => {
                    ...
                    row.innerHTML = '<div class=\'flex-1 grid grid-cols-1 sm:grid-cols-2 gap-2\'><input type=\'file\' name=\'doc_files[]\' class=\'w-full text-sm text-gray-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-gray-100 dark:file:bg-gray-600 file:text-gray-700 dark:file:text-gray-200 hover:file:bg-gray-200 dark:hover:file:bg-gray-500\'><input type=\'text\' name=\'doc_notes[]\' placeholder=\'Notiz zum Dokument...\' class=\'w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500\'></div><button type=\'button\' onclick=\'this.parentElement.remove()\' class=\'mt-1 text-red-400 hover:text-red-600\'><svg class=\'w-5 h-5\' fill=\'none\' stroke=\'currentColor\' viewBox=\'0 0 24 24\'><path stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'2\' d=\'M6 18L18 6M6 6l12 12\'/></svg></button>';
                    document.getElementById('doc-rows-edit').appendChild(row);
                })" class="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">+ Dokument hinzufügen</button>
            </div>
        ...
        <div class="mt-4 flex gap-3">
            ...
            <a href="{{ route('admin.tasks.show', $task) }}" class="px-5 py-2.5 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 text-sm font-medium rounded-lg border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700">Abbrechen</a>
        </div>
    </form>

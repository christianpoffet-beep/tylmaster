...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
    <form method="GET" action="{{ route('admin.tasks.index') }}" class="flex flex-wrap gap-2">
        ...
        <select name="priority" class="rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
        ...
        @if(request('search') || request('status') || request('priority'))
            <a href="{{ route('admin.tasks.index') }}" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Zurücksetzen</a>
        @endif
    </form>
    <a href="{{ route('admin.tasks.create') }}" class="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 whitespace-nowrap">+ Neue Aufgabe</a>
</div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($tasks as $task)
                    <tr class="hover:bg-gray-50 {{ $task->is_completed ? 'opacity-50' : '' }}">
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 text-sm">
                            <a href="{{ route('admin.tasks.show', $task) }}" class="font-medium {{ $task->is_completed ? 'line-through text-gray-400' : 'text-gray-900 hover:text-blue-600' }}">{{ $task->title }}</a>
                        </td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('admin.tasks.edit', $task) }}" class="text-sm text-blue-600 hover:text-blue-800">Bearbeiten</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-sm text-gray-500">Keine Aufgaben gefunden.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<div class="md:hidden space-y-2">
    @forelse($tasks as $task)
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 {{ $task->is_completed ? 'opacity-50' : '' }} {{ $task->isOverdue() ? 'border-l-4 border-l-red-400' : '' }}">
            <div class="flex items-start gap-3">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex flex-wrap items-center gap-2 mt-1.5 text-xs">
                        @if($task->priority)
                            @if($task->priority === 'high')
                                <span class="inline-flex items-center px-2 py-0.5 rounded font-medium bg-red-100 text-red-700">Hoch</span>
                            @elseif($task->priority === 'medium')
                                <span class="inline-flex items-center px-2 py-0.5 rounded font-medium bg-yellow-100 text-yellow-700">Mittel</span>
                            @else
                                <span class="inline-flex items-center px-2 py-0.5 rounded font-medium bg-gray-100 text-gray-600">Tief</span>
                            @endif
                        @endif
                        @if($task->due_date)

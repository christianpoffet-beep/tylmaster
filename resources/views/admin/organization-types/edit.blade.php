...
<div class="max-w-lg">
    <form method="POST" action="{{ route('admin.organization-types.update', $organizationType) }}">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-4">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $organizationType->name) }}" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div>
                ...
                <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $organizationType->sort_order) }}" min="0" class="w-24 rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
            </div>
        </div>
        ...
        <div class="mt-4 flex gap-3">
            ...
            <a href="{{ route('admin.organization-types.index') }}" class="px-5 py-2.5 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 text-sm font-medium rounded-lg border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700">Abbrechen</a>
        </div>
    </form>
    ...
    <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
        ...
        @if($usageCount > 0)
            <p class="text-sm text-gray-500 dark:text-gray-400">Dieser Typ wird von {{ $usageCount }} Organisation(en) verwendet und kann nicht gelöscht werden.</p>
        @else

...
@section('title', 'Organisationstyp erstellen')

@section('content')
<div class="max-w-lg">
    <form method="POST" action="{{ route('admin.organization-types.store') }}">
        @csrf
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-4">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="z.B. Agentur, Verlag..." class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div>
                ...
                <select name="color" id="color" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
                    @foreach($colorOptions as $value => $label)
                        <option value="{{ $value }}" {{ old('color', 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300') === $value ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>

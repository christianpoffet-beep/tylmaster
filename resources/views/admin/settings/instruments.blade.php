...
@section('title', 'Instrumente')

@section('content')
<div class="max-w-4xl">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Instrumente verwalten</h1>
    </div>

    <form method="POST" action="{{ route('admin.settings.instruments.update') }}" x-data="instrumentsEditor()">
        @csrf
            ...
            <template x-for="(group, gi) in groups" :key="gi">
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                    ...
                    <div class="flex items-center gap-3 p-4 border-b border-gray-200 dark:border-gray-700">
                        <input type="text" x-model="group.name" :name="'groups[' + gi + '][name]'" placeholder="Gruppenname (z.B. Saiteninstrumente)"
                    ...
                    <div class="p-4 space-y-2">
                        <template x-for="(item, ii) in group.items" :key="ii">
                            <div class="flex items-center gap-2">
                                <input type="text" x-model="group.items[ii]" :name="'groups[' + gi + '][items][' + ii + ']'" placeholder="Instrumentname"
        ...
        <div class="mt-6 flex gap-3">
            ...
            <a href="{{ route('admin.settings.system') }}" class="px-5 py-2.5 bg-white text-gray-700 text-sm font-medium rounded-lg border border-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50 dark:bg-gray-700/50">Abbrechen</a>
        </div>
    </form>
</div>

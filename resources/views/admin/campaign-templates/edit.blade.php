...
<form method="POST" action="{{ route('admin.campaign-templates.update', $template) }}" class="max-w-4xl">
    @csrf @method('PUT')

    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-6">
        <div class="flex items-center justify-between">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ $template->name }}</h2>
            <span class="text-xs text-gray-400">Slug: {{ $template->slug }}</span>
        </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="type" id="type" required
                            class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
                        <option value="email" {{ old('type', $template->type) === 'email' ? 'selected' : '' }}>E-Mail</option>
                        <option value="letter" {{ old('type', $template->type) === 'letter' ? 'selected' : '' }}>Brief</option>
                    </select>
                </div>
                <div>
                    ...
                    <select name="language" id="language" required
                            class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
                        <option value="de" {{ old('language', $template->language) === 'de' ? 'selected' : '' }}>Deutsch</option>
                        <option value="en" {{ old('language', $template->language) === 'en' ? 'selected' : '' }}>English</option>
                        <option value="fr" {{ old('language', $template->language) === 'fr' ? 'selected' : '' }}>Français</option>
                        <option value="it" {{ old('language', $template->language) === 'it' ? 'selected' : '' }}>Italiano</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="body" id="body" rows="20"
                      class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm font-mono focus:border-blue-500 focus:ring-blue-500">{{ old('body', $template->body) }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <textarea name="notes" id="notes" rows="3"
                          class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">{{ old('notes', $template->notes) }}</textarea>
            </div>
            <div>

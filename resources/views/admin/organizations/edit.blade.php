...
@section('content')
<div class="max-w-3xl">
    <form method="POST" action="{{ route('admin.organizations.update', $organization) }}" enctype="multipart/form-data" x-data="{ orgType: '{{ old('type', $organization->type) }}', catalogItems: {{ json_encode(old('catalogs', $organization->catalogs ?? [''])) }} }">
        @csrf
        @method('PUT')
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-6">
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="3" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">{{ old('notes', $organization->notes) }}</textarea>
            </div>

            @include('admin.partials.track-search', ['selected' => $organization->tracks])

            @include('admin.partials.release-search', ['selected' => $organization->releases])
            ...
            @php $hasProjects = $organization->projects->count() > 0; @endphp
            <div x-data="{ open: {{ $hasProjects ? 'true' : 'false' }} }">
                ...
                <div x-show="open" class="space-y-6 pt-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $organization->phone) }}" class="w-full sm:w-1/2 rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mt-4 flex items-center gap-3">
            ...
            <a href="{{ route('admin.organizations.show', $organization) }}" class="px-5 py-2.5 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 text-sm font-medium rounded-lg border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700">Abbrechen</a>
        </div>
    </form>

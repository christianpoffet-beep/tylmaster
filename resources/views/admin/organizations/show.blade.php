...
<div class="flex flex-col lg:flex-row gap-6">
    <div class="flex-1">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex justify-between items-start mb-4">
                <div class="flex items-center gap-4">
                    @if($organization->avatar_path)
                        <img src="{{ Storage::url($organization->avatar_path) }}" alt="{{ $organization->primary_name }}" class="w-16 h-16 rounded-full object-cover border border-gray-200">
                    @else
                ...
                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium {{ $typeColors[$organization->type] ?? 'bg-gray-100 text-gray-600' }}">{{ $typeLabels[$organization->type] ?? $organization->type }}</span>
            </div>
            ...
            <div class="space-y-3 text-sm">
                ...
                <div>
                    ...
                    <a href="mailto:{{ $organization->email }}" class="text-blue-600 ml-1">{{ $organization->email }}</a>
                </div>
                @endif
                ...
                <div>
                    <span class="text-gray-500">UID/MWST-Nr.:</span>
            ...
            <div class="mt-4 pt-4 border-t border-gray-200">
                <h3 class="text-sm font-semibold text-gray-700 mb-1">Websites</h3>
                <div class="space-y-1">
                    @foreach($organization->websites as $url)
                        <a href="{{ $url }}" target="_blank" rel="noopener" class="block text-sm text-blue-600 hover:text-blue-800">{{ $url }}</a>
                    @endforeach
                </div>
            </div>
            @endif
            ...
            <div class="mt-4 pt-4 border-t border-gray-200">
                <h3 class="text-sm font-semibold text-gray-700 mb-1">Genres</h3>
            ...
            <div class="mt-4 pt-4 border-t border-gray-200">
                <h3 class="text-sm font-semibold text-gray-700 mb-1">Biografie</h3>
                <p class="text-sm text-gray-600 whitespace-pre-line">{{ $organization->biography }}</p>
            </div>
            @endif

            @if($organization->contacts->count())
            <div class="mt-4 pt-4 border-t border-gray-200">
                <h3 class="text-sm font-semibold text-gray-700 mb-2">Kontakte</h3>
                <div class="space-y-1">
                    @foreach($organization->contacts as $contact)
                        <a href="{{ route('admin.contacts.show', $contact) }}" class="block text-sm text-blue-600 hover:text-blue-800">{{ $contact->full_name }}</a>
                    @endforeach
                </div>
            </div>
            @endif
        ...
        <div class="mt-4 flex gap-3">
            <a href="{{ route('admin.organizations.edit', $organization) }}" class="px-4 py-2 bg-gray-800 text-white text-sm rounded-lg hover:bg-gray-700">Bearbeiten</a>

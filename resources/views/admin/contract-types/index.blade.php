...
@section('title', 'Vertragstypen')

@section('content')
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
    <form method="GET" action="{{ route('admin.contract-types.index') }}" class="flex gap-2">
        ...
        @if(request('search'))
            <a href="{{ route('admin.contract-types.index') }}" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 dark:hover:text-gray-200 dark:text-gray-100">Zurücksetzen</a>
        @endif
    </form>
    <a href="{{ route('admin.contract-types.create') }}" class="px-4 py-2 bg-blue-600 dark:bg-blue-700 text-white text-sm rounded-lg hover:bg-blue-700 dark:hover:bg-blue-600 whitespace-nowrap">+ Typ erstellen</a>
</div>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700/50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Verträge</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($types as $type)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 dark:bg-gray-700/50">
                        ...
                        <td class="px-4 py-3 text-right whitespace-nowrap">
                            <a href="{{ route('admin.contract-types.edit', $type) }}" class="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">Bearbeiten</a>
                            @if($type->usage_count === 0)
                            <form method="POST" action="{{ route('admin.contract-types.destroy', $type) }}" class="inline ml-2" onsubmit="return confirm('Typ wirklich löschen?')">
                                @csrf @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">Keine Vertragstypen gefunden.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>

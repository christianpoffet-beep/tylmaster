...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
    <form method="GET" action="{{ route('admin.contract-types.index') }}" class="flex gap-2">
        ...
        @if(request('search'))
            <a href="{{ route('admin.contract-types.index') }}" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Zurücksetzen</a>
        @endif
    </form>
    <a href="{{ route('admin.contract-types.create') }}" class="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 whitespace-nowrap">+ Typ erstellen</a>
</div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Verträge</th>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($types as $type)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-right whitespace-nowrap">
                            <a href="{{ route('admin.contract-types.edit', $type) }}" class="text-sm text-blue-600 hover:text-blue-800">Bearbeiten</a>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-sm text-gray-500">Keine Vertragstypen gefunden.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>

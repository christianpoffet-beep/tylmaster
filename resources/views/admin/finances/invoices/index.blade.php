...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
    <form method="GET" action="{{ route('admin.invoices.index') }}" class="flex flex-wrap gap-2">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Rechnungsnr. suchen..." class="rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($invoices as $invoice)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 dark:bg-gray-700/50">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">{{ $invoice->due_date ? $invoice->due_date->format('d.m.Y') : '-' }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('admin.invoices.edit', $invoice) }}" class="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">Bearbeiten</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="8" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">Keine Rechnungen gefunden.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
@if($accountings->count())
<div class="mt-8">
    <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Buchungsjournale</h3>

...
@section('title', 'Rechnung bearbeiten')
...
<div class="max-w-3xl" x-data="invoiceForm()">
    ...
    <form method="POST" action="{{ route('admin.invoices.update', $invoice) }}" enctype="multipart/form-data" @submit="submitting = true">
        ...
        <div class="mt-4 flex gap-3">
            ...
            <a href="{{ route('admin.invoices.show', $invoice) }}" x-show="!submitting" class="px-5 py-2.5 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 text-sm font-medium rounded-lg border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700">Abbrechen</a>
        </div>
    </form>
...
@php
    $senderTypeDefault = $invoice->sender_organization_id ? 'organization' : ($invoice->sender_contact_id ? 'contact' : '');
    $recipientTypeDefault = $invoice->organization_id ? 'organization' : 'contact';
@endphp
...
function invoiceForm() {
    return {
        ...
        accounts: [],
        debitAccountId: @json(old('debit_account_id', $invoice->debit_account_id ?? '')),
        creditAccountId: @json(old('credit_account_id', $invoice->credit_account_id ?? '')),
        senderType: @json(old('sender_type', $senderTypeDefault)),
        senderContactId: @json(old('sender_contact_id', $invoice->sender_contact_id ?? '')),
        senderOrganizationId: @json(old('sender_organization_id', $invoice->sender_organization_id ?? '')),
        orgContactsMap: @json($orgContactsMap),
        senderOrgContacts: [],
        recipientType: @json(old('recipient_type', $recipientTypeDefault)),
        recipientContactId: @json(old('contact_id', $invoice->contact_id ?? '')),
        recipientOrganizationId: @json(old('organization_id', $invoice->organization_id ?? '')),
        recipientOrgContacts: [],
        init() {
            const savedSenderContactId = this.senderContactId;
            const savedRecipientContactId = this.recipientContactId;
            // Load org contacts without clearing saved selections
            ...
            // Restore contact selections after x-for options render
            this.$nextTick(() => {
                this.senderContactId = savedSenderContactId;
                this.recipientContactId = savedRecipientContactId;
            });
            if (this.accountingId) this.loadAccounts(this.accountingId);
        },
        onSenderOrgChange() {
        ...
        async loadAccounts(accountingId) {
            if (!accountingId) { this.accounts = []; return; }
            try {
                const resp = await fetch(`/admin/accountings/${accountingId}/accounts`);
        ...
        async onAccountingChange() {
            ...
            await this.loadAccounts(this.accountingId);
        },
        addItem() {

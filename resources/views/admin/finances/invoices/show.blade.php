...
<div class="max-w-3xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex justify-between items-start mb-6">
            <div>
                ...
                <div class="mt-2 flex items-center gap-2">
                    ...
                    @endswitch
                    @if($invoice->template)
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-600">{{ $invoice->template->name }}</span>
                    @endif
                    @if($invoice->accounting)
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-indigo-100 text-indigo-700">{{ $invoice->accounting->name }}</span>
                    @endif
                    @if($invoice->project)
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-700">{{ $invoice->project->name }}</span>
                    @endif
                </div>
            </div>
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
            <div>
                <dt class="text-sm font-medium text-gray-500">Absender</dt>
                <dd class="mt-1 text-sm text-gray-900">
                    @if($invoice->senderOrganization)
                        <a href="{{ route('admin.organizations.show', $invoice->senderOrganization) }}" class="text-blue-600 hover:text-blue-800">{{ $invoice->senderOrganization->primary_name }}</a>
                        @if($invoice->senderContact)
                            <span class="text-gray-500">—</span>
                            <a href="{{ route('admin.contacts.show', $invoice->senderContact) }}" class="text-blue-600 hover:text-blue-800">{{ $invoice->senderContact->full_name }}</a>
                        @endif
            ...
            <div>
                <dt class="text-sm font-medium text-gray-500">Empfänger</dt>
                <dd class="mt-1 text-sm text-gray-900">
                    @if($invoice->organization)
                        <a href="{{ route('admin.organizations.show', $invoice->organization) }}" class="text-blue-600 hover:text-blue-800">{{ $invoice->organization->primary_name }}</a>
                        @if($invoice->contact)
                            <span class="text-gray-500">—</span>
                            <a href="{{ route('admin.contacts.show', $invoice->contact) }}" class="text-blue-600 hover:text-blue-800">{{ $invoice->contact->full_name }}</a>
                        @endif
            ...
            <div>
                ...
                <dd class="mt-1 text-lg font-bold text-gray-900">{{ number_format($invoice->amount, 2, '.', "'") }} {{ $invoice->currency }}</dd>
            ...
            <div>
                ...
                <dd class="mt-1 text-sm text-gray-900">{{ $invoice->invoice_date ? $invoice->invoice_date->format('d.m.Y') : '—' }}</dd>
            </div>
            <div>
                <dt class="text-sm font-medium text-gray-500">Fälligkeitsdatum</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $invoice->due_date ? $invoice->due_date->format('d.m.Y') : '—' }}</dd>
            </div>
            @if($invoice->project)
            <div>
                <dt class="text-sm font-medium text-gray-500">Projekt</dt>
                <dd class="mt-1 text-sm text-gray-900">
                    <a href="{{ route('admin.projects.show', $invoice->project) }}" class="text-blue-600 hover:text-blue-800">{{ $invoice->project->name }}</a>
                </dd>
            </div>
            @endif
            @if($invoice->debitAccount || $invoice->creditAccount)
            <div>
                <dt class="text-sm font-medium text-gray-500">Soll-Konto</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $invoice->debitAccount ? $invoice->debitAccount->number . ' ' . $invoice->debitAccount->name : '—' }}</dd>
            </div>
            <div>
                <dt class="text-sm font-medium text-gray-500">Haben-Konto</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $invoice->creditAccount ? $invoice->creditAccount->number . ' ' . $invoice->creditAccount->name : '—' }}</dd>
            </div>
            @endif
        </dl>
                    ...
                    <tfoot>
                        @if($invoice->vat_rate && $invoice->vat_rate > 0)
                            ...
                            <tr class="border-t border-gray-300">
                                <td colspan="4" class="py-2 text-sm font-medium text-gray-700 text-right">Total {{ $invoice->currency }}</td>
    ...
    <div class="mt-4">
        <a href="{{ route('admin.invoices.index') }}" class="text-sm text-gray-500 hover:text-gray-700">&larr; Zurück zur Übersicht</a>
    </div>
</div>
@endsection

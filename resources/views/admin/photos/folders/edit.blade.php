...
<div class="max-w-3xl">
    <div class="mb-4">
        <a href="{{ route('admin.photos.folders.show', $folder) }}" class="text-sm text-gray-500 hover:text-gray-700">&larr; Zurück zum Ordner</a>
    </div>
    ...
    <form method="POST" action="{{ route('admin.photos.folders.update', $folder) }}">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $folder->name) }}" required class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Beschreibung</label>
                <textarea name="description" id="description" rows="3" class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $folder->description) }}</textarea>
            </div>

            <div>
                <label for="parent_id" class="block text-sm font-medium text-gray-700 mb-1">Übergeordneter Ordner</label>
                <select name="parent_id" id="parent_id" class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                <div class="max-h-36 overflow-y-auto border border-gray-200 rounded-lg p-3 space-y-2">
                    @foreach($organizations as $org)
                        <label class="flex items-center">
                            ...
                            <span class="ml-2 text-sm text-gray-700">{{ $org->primary_name }}</span>
                        </label>
                    @endforeach
                </div>
            </div>
        ...
        <div class="mt-4 flex gap-3">
            ...
            <a href="{{ route('admin.photos.folders.show', $folder) }}" class="px-5 py-2.5 bg-white text-gray-700 text-sm font-medium rounded-lg border border-gray-300 hover:bg-gray-50">Abbrechen</a>
        </div>
    </form>
</div>
@endsection

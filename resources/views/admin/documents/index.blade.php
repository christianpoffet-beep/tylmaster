...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
    <form method="GET" action="{{ route('admin.documents.index') }}" class="flex flex-wrap gap-2">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Suche in Titel & Notizen..." class="rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500 w-48">
        <select name="category" class="rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
            <option value="">Alle Kategorien</option>
            @foreach($catLabels as $v => $l)
                <option value="{{ $v }}" {{ request('category') === $v ? 'selected' : '' }}>{{ $l }}</option>
            @endforeach
        </select>
        <select name="source" class="rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
        ...
        @if(request('search') || request('category') || request('source'))
            <a href="{{ route('admin.documents.index') }}" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Zurücksetzen</a>
        @endif
    </form>
    <a href="{{ route('admin.documents.create') }}" class="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 whitespace-nowrap">+ Dokument hochladen</a>
</div>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($documents as $doc)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                ...
                                <div class="min-w-0">
                                    ...
                                    @endphp
                                    @if($isPreviewable)
                                        <button type="button" class="text-sm font-medium text-gray-900 hover:text-blue-600 truncate block text-left"
                                    ...
                                    @else
                                        <a href="{{ route('admin.documents.download', $doc) }}" class="text-sm font-medium text-gray-900 hover:text-blue-600 truncate block">{{ $doc->title }}</a>
                                    @endif
                                    @if($doc->notes)
                        ...
                        <td class="px-4 py-3 text-xs text-gray-500">{{ $doc->mime_type ? Str::afterLast($doc->mime_type, '/') : '-' }}</td>
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-600">{{ $catLabels[$doc->category] ?? $doc->category }}</span>
                        </td>
                        <td class="px-4 py-3">
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-sm text-gray-500">Keine Dokumente gefunden.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>

    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div class="space-y-3 text-sm">
            <div class="flex items-center">
                <span class="w-32 text-gray-500 dark:text-gray-400">Kategorie:</span>
                @php $catLabels = ['contract' => 'Vertrag', 'invoice' => 'Rechnung', 'legal' => 'Rechtliches', 'music' => 'Musik', 'other' => 'Sonstiges']; @endphp
                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">{{ $catLabels[$document->category] ?? $document->category }}</span>
            </div>
            <div class="flex items-center">
                <span class="w-32 text-gray-500 dark:text-gray-400">Dateigrösse:</span>
                <span class="text-gray-900 dark:text-gray-100">{{ $document->file_size ? number_format($document->file_size / 1024, 0) . ' KB' : '-' }}</span>
            </div>
            <div class="flex items-center">
                <span class="w-32 text-gray-500 dark:text-gray-400">Dateityp:</span>
                <span class="text-gray-900 dark:text-gray-100">{{ $document->mime_type ?? '-' }}</span>
            </div>
            <div class="flex items-center">
                <span class="w-32 text-gray-500 dark:text-gray-400">Dateiname:</span>
                <span class="text-gray-900 dark:text-gray-100">{{ $document->original_filename ?? '-' }}</span>
            </div>
            <div class="flex items-center">
                <span class="w-32 text-gray-500 dark:text-gray-400">Hochgeladen am:</span>
                <span class="text-gray-900 dark:text-gray-100">{{ $document->created_at->format('d.m.Y H:i') }}</span>
            </div>
            <div class="flex items-center">
                <span class="w-32 text-gray-500 dark:text-gray-400">Aktualisiert am:</span>
                <span class="text-gray-900 dark:text-gray-100">{{ $document->updated_at->format('d.m.Y H:i') }}</span>
            </div>
        </div>
    </div>

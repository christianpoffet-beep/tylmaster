...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
    <form method="GET" action="{{ route('admin.invoice-templates.index') }}" class="flex gap-2">
        ...
        @if(request('search'))
            <a href="{{ route('admin.invoice-templates.index') }}" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Zurücksetzen</a>
        @endif
    </form>
    <a href="{{ route('admin.invoice-templates.create') }}" class="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 whitespace-nowrap">+ Neue Vorlage</a>
</div>
                ...
                @forelse($templates as $tpl)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500">
                            ...
                            @elseif($tpl->contact)
                                <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-purple-100 text-purple-700 ml-1">Kontakt</span>
                            @endif
                        </td>
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('admin.invoice-templates.edit', $tpl) }}" class="text-sm text-blue-600 hover:text-blue-800">Bearbeiten</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-sm text-gray-500">Keine Vorlagen vorhanden.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>

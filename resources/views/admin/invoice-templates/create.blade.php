...
@section('title', 'Neue Rechnungsvorlage')

@section('content')
<div class="max-w-3xl" x-data="templateForm()">
    <div class="mb-6">
        <h2 class="text-xl font-bold text-gray-900 dark:text-gray-100">Neue Rechnungsvorlage</h2>
        <p class="text-sm text-gray-500 mt-1">Absender-Profil mit Standardpositionen</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.invoice-templates.store') }}" enctype="multipart/form-data">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-6">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500" placeholder="z.B. The Yelling Light GmbH">
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="vat_rate" id="vat_rate" value="{{ old('vat_rate') }}" step="0.01" min="0" max="100" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500" placeholder="z.B. 8.10">
                    ...
                    <div>
                        ...
                        <input type="number" name="payment_terms_days" id="payment_terms_days" value="{{ old('payment_terms_days', 30) }}" required min="0" max="365" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>
                </div>
            </div>
            ...
            {{-- Logo --}}
            <div class="border-t border-gray-200 dark:border-gray-700 pt-6" x-data="{ logoSource: '{{ old('logo_source', 'avatar') }}' }">
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <textarea name="footer_text" id="footer_text" rows="2" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 text-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Wird am Ende der Rechnung angezeigt">{{ old('footer_text') }}</textarea>
            </div>
        </div>

        <div class="mt-4 flex items-center gap-3">
            <button type="submit" class="px-5 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700">Erstellen</button>
            <a href="{{ route('admin.invoice-templates.index') }}" class="px-5 py-2.5 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 text-sm font-medium rounded-lg border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700">Abbrechen</a>
        </div>
    </form>
</div>
...
function templateForm() {
    return {
        items: @json(old('template_items', [])),
        orgContactsMap: @json($orgContactsMap),
        senderType: @json(old('sender_type', '')),
        senderContactId: @json(old('contact_id', '')),
        senderOrganizationId: @json(old('organization_id', '')),
        senderOrgContacts: [],
        recipientType: @json(old('recipient_type', '')),
        recipientContactId: @json(old('recipient_contact_id', '')),
        recipientOrganizationId: @json(old('recipient_organization_id', '')),

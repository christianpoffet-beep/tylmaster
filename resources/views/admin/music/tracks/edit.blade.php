...
<div class="max-w-3xl">
    <form method="POST" action="{{ route('admin.tracks.update', $track) }}" enctype="multipart/form-data">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $track->title) }}" required class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="isrc" class="block text-sm font-medium text-gray-700 mb-1">ISRC</label>
                    <input type="text" name="isrc" id="isrc" value="{{ old('isrc', $track->isrc) }}" class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="genre" id="genre" value="{{ old('genre', $track->genre) }}" class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('genre') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="duration_seconds" class="block text-sm font-medium text-gray-700 mb-1">Dauer (Sekunden)</label>
                    <input type="number" name="duration_seconds" id="duration_seconds" value="{{ old('duration_seconds', $track->duration_seconds) }}" min="0" class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <label for="release_id" class="block text-sm font-medium text-gray-700 mb-1">Release</label>
                    <select name="release_id" id="release_id" class="w-full rounded-lg border-gray-300 text-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            <div>
                <label for="audio_file" class="block text-sm font-medium text-gray-700 mb-1">Audio-Datei</label>
        ...
        <div class="mt-4 flex gap-3">
            ...
            <a href="{{ route('admin.tracks.show', $track) }}" class="px-5 py-2.5 bg-white text-gray-700 text-sm font-medium rounded-lg border border-gray-300 hover:bg-gray-50">Abbrechen</a>
        </div>
    </form>

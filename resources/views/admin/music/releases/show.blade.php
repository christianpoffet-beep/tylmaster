...
<div class="max-w-3xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex justify-between items-start mb-6">
            ...
            <div class="flex gap-2">
                <a href="{{ route('admin.releases.edit', $release) }}" class="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">Bearbeiten</a>
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
            ...
            <div>
                <dt class="text-sm font-medium text-gray-500">Release-Datum</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $release->release_date ? $release->release_date->format('d.m.Y') : '-' }}</dd>
            </div>
            <div>
                <dt class="text-sm font-medium text-gray-500">Label</dt>
                <dd class="mt-1 text-sm text-gray-900">{{ $release->label ?? '-' }}</dd>
            </div>
        </dl>
    </div>
    ...
    <x-admin.collapsible-card title="Tracks" :count="$release->tracks->count()" class="mt-6">
                ...
                @foreach($release->tracks as $index => $track)
                    <div class="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg">
                        <div class="flex items-center gap-3">
                            <span class="text-sm text-gray-400 font-mono w-6 text-right">{{ $index + 1 }}</span>
        ...
        @else
            <p class="text-sm text-gray-500">Keine Tracks zugewiesen.</p>
        @endif
    </x-admin.collapsible-card>

    <div class="mt-4">
        <a href="{{ route('admin.releases.index') }}" class="text-sm text-gray-500 hover:text-gray-700">&larr; Zurück zur Übersicht</a>
    </div>
</div>
@endsection
